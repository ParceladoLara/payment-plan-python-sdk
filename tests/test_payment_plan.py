...
from payment_plan import disbursement_date_range, next_disbursement_date, get_non_business_days_between,Response, Params, calculate_payment_plan,calculate_down_payment_plan,DownPaymentParams,DownPaymentResponse
...
class TestPaymentPlanUtilities(unittest.TestCase):
    ...
    def test_calculate_down_payment_plan(self):
        expected = [
            DownPaymentResponse(
                ...
                plans=[
                    Response(
                        installment=1,
                        due_date=datetime(2025, 6, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 5, 9, tzinfo=timezone(timedelta(hours=-3))),
                        accumulated_days=25,
                        days_index=0.981371965896169,
                        accumulated_days_index=0.981371965896169,
                        interest_rate=0.0235,
                    ...
                    Response(
                        installment=2,
                        due_date=datetime(2025, 7, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 5, 9, tzinfo=timezone(timedelta(hours=-3))),
                        accumulated_days=55,
                        days_index=0.958839243657051,
                        accumulated_days_index=1.94021120955322,
                    ...
                    Response(
                        installment=3,
                        due_date=datetime(2025, 8, 4, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 5, 9, tzinfo=timezone(timedelta(hours=-3))),
                        accumulated_days=87,
                    ...
                    Response(
                        installment=4,
                        due_date=datetime(2025, 9, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 5, 9, tzinfo=timezone(timedelta(hours=-3))),
            ...
            DownPaymentResponse(
                ...
                plans=[
                    Response(
                        installment=1,
                        due_date=datetime(2025, 7, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 6, 9, tzinfo=timezone(timedelta(hours=-3))),
                        accumulated_days=24,
                        days_index=0.981371965896169,
                        accumulated_days_index=0.981371965896169,
                        interest_rate=0.0235,
                    ...
                    Response(
                        installment=4,
                        due_date=datetime(2025, 10, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 6, 9, tzinfo=timezone(timedelta(hours=-3))),
            ...
            DownPaymentResponse(
                ...
                plans=[
                    Response(
                        installment=1,
                        due_date=datetime(2025, 8, 4, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 7, 9, tzinfo=timezone(timedelta(hours=-3))),
                        accumulated_days=26,
                    ...
                    Response(
                        installment=4,
                        due_date=datetime(2025, 11, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 7, 9, tzinfo=timezone(timedelta(hours=-3))),
                        accumulated_days=117,
                        days_index=0.912281747198563,
            ...
            DownPaymentResponse(
                ...
                plans=[
                    Response(
                        installment=1,
                        due_date=datetime(2025, 9, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 8, 11, tzinfo=timezone(timedelta(hours=-3))),
                        accumulated_days=23,
                        days_index=0.981371965896169,
                        accumulated_days_index=0.981371965896169,
                        interest_rate=0.0235,
                    ...
                    Response(
                        installment=4,
                        due_date=datetime(2025, 12, 3, tzinfo=timezone(timedelta(hours=-3))),
                        disbursement_date=datetime(2025, 8, 11, tzinfo=timezone(timedelta(hours=-3))),
        ...
        for i, (dr, de) in enumerate(zip(resp, expected)):
            self.assertEqual(dr.installment_amount, de.installment_amount, f"Installment {i + 1}: InstallmentAmount mismatch")
            self.assertEqual(dr.total_amount, de.total_amount, f"Installment {i + 1}: TotalAmount mismatch")
            self.assertEqual(dr.installment_quantity, de.installment_quantity, f"Installment {i + 1}: InstallmentQuantity mismatch")

            for j, (r, e) in enumerate(zip(dr.plans, de.plans)):
                self.assertEqual(r.installment, e.installment, f"Installment {i + 1}: Installment mismatch")
                self.assertEqual(r.accumulated_days, e.accumulated_days, f"Installment {i + 1}: AccumulatedDays mismatch")
                self.assertAlmostEqual(r.days_index, e.days_index, places=6, msg=f"Installment {i + 1}: DaysIndex mismatch")
                self.assertAlmostEqual(
